...
async function main() {
  ...
  if (noCat.length > 0) {
    ...
    noCat.slice(0, 20).forEach(p => console.log(`  • ${p.namePl}`));
  ...
  Object.entries(byCategory)
    ...
    .forEach(([cat, prods]) => {
      console.log(`\n${cat} (${prods.length}):`);
      prods.slice(0, 5).forEach(p => console.log(`  • ${p.namePl}`));
  ...
  for (const p of products) {
    ...
    } else if ((name.includes("broszura") || name.includes("książka")) && p.category.namePl !== "Broszury i materiały") {
