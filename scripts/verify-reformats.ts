...
async function verify() {
  const products = await prisma.product.findMany({
    where: {
      ...
      slug: { in: ["ahista", "aloevera-dr-jacob-s", "kurkumina-liposomalna", "omni-logic-immune-wspiera-system-odpornosciowy-witamina-d-do-31-10-2026"] }
    },
    select: {
      slug: true,
      namePl: true,
  ...
  products.forEach(p => {
    ...
    if (p.benefitsPl) {
      p.benefitsPl.slice(0, 3).forEach(b => console.log(`  • ${b}`));
    }
  });
